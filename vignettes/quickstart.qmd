...
mydata <- load_excel_file(trial_file = system.file("extdata","standard_exemple.xlsx",package="startbox"))
```
...
```{r}
# affichage des premières lignes des données d'observation data_G1_2024 :
...
```{r}
plot_xpheat(mydata,stat = "eff_G1", calculation_choices = "efficacité frequence PM_BER_PC", resids = T)
```
# Exporter les stats

Les résultats d'analyses stats peuvent être exportés dans un fichier Excel. 
Le principe de base est de séparer strictement d'une part les données brutes et d'autre part les données traitées et les résultats d'analyses stats. La procédure d'export créée donc un nouveau fichier Excel. Ce fichier Excel contient les données préparées et les statistiques associées. Il est par défaut sauvegardé dans le dossier téléchargement.

```{r echo = TRUE,  eval=FALSE}
export_stats_sheets(mydata)
```
