...
```{r eval= FALSE}
# Option 1 : ajout direct dans l'objet user_data avec le nom du fichier
mydata$add_obs(name = "F1", df = data_F1, source_file = file_path)
```

```{r}
# Option 2 : importer directement dans mydata
import_topvigne_csv(mydata,filepath=file_path,obs_name = "F1")
```
