...
#' @param block colname to use as block. if FALSE : no block factor tested. if TRUE : "block_code" is used. if value : value is used
...
test_stats <- function(
  ...
  for (calc in unique_calculation) {
    ...
    temp_stat <- tryCatch(
      ...
      error = function(e) {
        message("Error : non-computable statistics for ", calc)
        return(NULL)
      }
    )
...
stat_on_data <- function(
  ...
  for (var in predictors) {
    ...
    if (length(levels(data[[var_name]])) < 2) {
      stop(
        "contrasts can be applied only to factors with 2 or more levels. Function aborted"
      )
    }
  }
  ...
  if (length(formula_vars) > 2) {
    ...
    model_with_interaction <- stats::lm(
  ...
  df.stats %>%
    dplyr::mutate(
      signif = dplyr::case_when(
        !is.na(ptest) & ptest <= 0.001 ~ "***",
        !is.na(ptest) & ptest <= 0.01 ~ "**",
        !is.na(ptest) & ptest <= 0.05 ~ "*",
        !is.na(ptest) & ptest > 0.5 ~ "ns",
        TRUE ~ ""
      )
    ) -> df.stats
...
check_normality <- function(model, alpha = 0.05) {
  ...
  if (is.na(pval)) {
    message("for",response_name,"Shapiro test non calculable : p_norm forced to 0")
    pval <- 0
  }

  if (pval < alpha) {
    message(paste("for",response_name,
...
check_heteroscedasticity <- function(model, alpha = 0.05) {
  ...
  if (is.na(pval)) {
    message("Bartlett test non calculable : p_var forced to 0")
    pval <- 0
  }
  if (pval < alpha) {
    message(paste("for",response_name,
